    ...
    def scan_network(
        ...
        with self._scan_lock:
            ...
            for d in all_devices.values():
                # Extract numeric ports if available
                ports = []
                if hasattr(d, 'open_ports') and d.open_ports:
                    ports = [p['port'] for p in d.open_ports] if isinstance(d.open_ports[0], dict) else d.open_ports

                fp = self.fingerprinter.fingerprint(
                    mac=d.mac, hostname=d.hostname, ttl=d.ttl, open_ports=ports
                )
                # We need to map fingerprint fields to DiscoveredDevice (which needs them for to_dict)
                d.vendor = fp.vendor
                d.device_type = fp.device_type
                d.os_hint = fp.os_hint
                # Inject icon/label into hostname if needed, or just keep them for the to_dict override
                d.device_icon = fp.to_dict()["device_icon"]
